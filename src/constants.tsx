...
const Color: React.FC<ColorProps> = ({ color, dispatch, editColor }) => {
  return (<div className="color" onClick={() => editColor(color)}>
    <div className="preview" style={{ background: color.hex }}>
    </div>
  ...
  return (<div className="page constants">
    <section>
      <div className="constant-header">
        ...
        <h1>Colors</h1>
        <FAB.Button small icon="plus" onClick={() => {
          colorModal.open({ name: '', hex: '#00ff00' })
        }} />
      </div>
      <div className="color-list">
        {console.log(colors)}
        {colors.map((c: Color) => <Color
          editColor={() => colorModal.open(c)}
          dispatch={dispatch} color={c} key={c.name} />)}
      </div>
    </section>
    <section>
      <KeywordDialog modal={keywordModal}
        onClose={() => {
          keywordModal.close()
        }}
        onSave={(keyword) => {
