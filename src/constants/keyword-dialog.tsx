...
import { Dialog, Button, Forms } from '../ui'
import { type } from 'os'
...
export const CardTypeDialog: React.FC<CardTypeDialogProps> = ({ children, modal, onSave, onClose }) => {
  ...
  return (<Dialog.Dialog open={modal.isOpen}>
    ...
    <Dialog.Footer>
      <Button variant="solid" color="secondary" label="Cancel" onClick={onClose} />
      <Button variant="solid" color="primary" label="Save"
        onClick={() => {
          onSave({ label, value, description, id: keyword && keyword.id || undefined })
        }} />
    </Dialog.Footer>
  </Dialog.Dialog>)
}
